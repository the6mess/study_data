...
{   
    debug_printf("\n\r");
    debug_printf("******************************\n\r");
    debug_printf("1-normaly boot\n\r");
    debug_printf("2-swap app\n\r");
    debug_printf("3-backtrace\n\r");
    debug_printf("4-accept upload\n\r");
	debug_printf("5-lpuart upload image\n\r");
    debug_printf("******************************\n\r");
    debug_printf("input operation:\r\n");
    ...
    case 50:
    {
       status = update_image_state(kSwapType_ReadyForTest);
       debug_printf("update_image_state(%x): %x\r\n", kSwapType_ReadyForTest, status);
       break;
    }   
    case 51:
    {  
       status = update_image_state(kSwapType_Rollback);
       debug_printf("update_image_state(%x): %x\r\n", kSwapType_Test, kSwapType_Rollback);
       break;
    }  
    case 52:
    {
       status = update_image_state(kSwapType_Permanent);
       debug_printf("update_image_state(%x): %x\r\n", kSwapType_Permanent, status);
       break;
    }
	case 53:
    {
        SDK_DelayAtLeastUs(1000*7000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
        result = xmodemUpload();
        if(!result)
        debug_printf("lpuart upload failed\n\r");
    ...
    default:
        debug_printf("enter normal boot mode\n\r");
        break;
    }
